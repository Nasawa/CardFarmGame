function multiCharCreate(characterInitObj)
{
	console.log("Making new character...");
	...
	characterInitObj.addComponent("2D, DOM, female, Keyboard, SpriteAnimation, charimg2, persist, player")
		...
		{	
			$(".charimg").each(function(index)
			{
				if(characterInitObj._tempy > 0)
				{
					...
					if($(this).hasClass('shoe'))
					{
						...
						//characterInitObj._multitoward = false;
				}
				else
				{
					if($(this).hasClass('hair'))
			...
			if (characterInitObj._multitoward)//if multitoward is not null or false
			{
				if (this.y < characterInitObj._multitoward.split("?")[1] - 100) 
				{
					this.y += characterInitObj._speed/2;//set y
					characterInitObj._tempy = -1;
				}
				else if (this.y > characterInitObj._multitoward.split("?")[1] - 100)
				{
					this.y -= characterInitObj._speed/2;
					characterInitObj._tempy = 1;
				}
					
				if (this.x < characterInitObj._multitoward.split("?")[0])//Split by the question mark
				{
				...
				else if (this.x > characterInitObj._multitoward.split("?")[0])
				{
				...
				{
					/**this.stop();//if there is no multitowards, stop moving
					if(tempy > 0)
				...
				if (this.x == characterInitObj._multitoward.split("?")[0] && this.y == characterInitObj._multitoward.split("?")[1] - 100) 
				{
					if(this.x != characterInitObj._playertile.x || this.y != characterInitObj._playertile.y - 100)
						characterInitObj._multitoward = characterInitObj._playertile.x + "?" + characterInitObj._playertile.y;
					else
					{
						characterInitObj._multitoward2 = characterInitObj._multitoward;
						characterInitObj._multitoward = false;//Set multitoward to false if the conditions say we're there.
						this.stop();
			...
			else//reset tiles if we're done moving multitoward them
			{
				/**if (characterInitObj._playertile.has('grass')) 
